...
| `GET /api/settings` | settings.manage | `{company_name, primary_color, logo_url, stalled_days, event_email_mode, alert_emails: [], lecom_supply_form_url, lecom_portal_url, lecom_api_base_url, lecom_process_id, lecom_process_version, lecom_instance_form_url, lecom_api_start_url, lecom_api_x_server, lecom_workspace_start_url}` |
| `PUT /api/settings` | settings.manage | any subset: `company_name` (≤100), `primary_color` (`#RRGGBB`), `stalled_days` (1–60), `event_email_mode` (`por_retirada`/`consolidado`), `alert_emails` (array or "a@b.com, c@d.com"), `lecom_supply_form_url` (template with `{id}`, `{code}`, `{name}`, `{quantity}`, `{unit_value}`, `{category}`), `lecom_portal_url`, `lecom_api_base_url`, `lecom_process_id`, `lecom_process_version` |
